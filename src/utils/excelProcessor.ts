...
/**
 * Smart matching for SKU supporting classification/classification suffixes.
 * The reference pricing SKU (from the new pricing file, e.g. "loa A36") is used to search the original SKU
 * (which can contain trailing descriptions like "loa A36 Đen", "loa A36 Vàng").
 * To avoid incorrect matches, we require a word boundary suffix (space, hyphen, slash, etc.) and
 * choose the most specific (longest) matching reference SKU first.
 */
export function findBestSkuMatch(
  ...
  // 2. Trailing classification matching: Find the longest reference SKU prefix that is followed by a word boundary.
  let bestMatchKey: string | null = null;
  let bestMatchLen = 0;

  for (const refSku of refMap.keys()) {
    if (sourceSku.startsWith(refSku)) {
      const nextChar = sourceSku.charAt(refSku.length);
      // It must be at the end of the string, or followed by a non-alphanumeric word boundary character
      const isWordBoundary = !nextChar || !/[a-zA-Z0-9\u00C0-\u1EFF]/.test(nextChar);

      if (isWordBoundary) {
        if (refSku.length > bestMatchLen) {
          bestMatchLen = refSku.length;
          bestMatchKey = refSku;
        }
      }
    }
  ...
  if (bestMatchKey) {
    return { matchedSku: bestMatchKey, value: refMap.get(bestMatchKey) };
  }
...
export function createMockupFiles(): { sourceFile: FileData; refFile: FileData } {
  ...
  const sourceRaw: any[][] = [
    ...
    ['SP005', 'Loa Bluetooth A36 Đen', 'Còn hàng', 'loa A36 Đen', '650000.00', '600000.00'],
    ['SP005', 'Loa Bluetooth A36 Vàng', 'Còn hàng', 'loa A36 Vàng', '650000.00', '600000.00']
  ];
  ...
  const refRaw: any[][] = [
    ...
    ['loa A36', '499000.00'], // New reference price for Loa Bluetooth A36 that will match both "loa A36 Đen" and "loa A36 Vàng" from original file!
